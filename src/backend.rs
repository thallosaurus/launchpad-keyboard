use std::{collections::HashMap, iter::Map, sync::Arc, thread::JoinHandle, time::Duration};
...
use crate::{mapping::Actions, message::{Message, MidiChannel, MidiMessage, MidiVelocity, Note}};

type ActiveActions = HashMap<Actions, MidiVelocity>;

pub async fn event_loop(mut from_raw_device: Receiver<Message>, mut output_port: MidiOutputConnection) -> Result<(), RecvError> {
    let (tx, rx) = channel(1);
